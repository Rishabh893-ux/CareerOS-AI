...
export default function Sidebar({
  ...
  return (
    <aside className={`relative z-30 flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out
      bg-surface-alt border-r border-line elevated
      ...
      <div className={`h-16 flex items-center border-b border-line px-4 gap-3`}>
        ...
        {sidebarOpen && (
          <div className="flex-1 overflow-hidden">
            ...
            <span className="text-[9px] text-muted font-semibold uppercase tracking-widest">Intelligence Suite</span>
          </div>
        )}
        <button onClick={onToggleSidebar}
          className="p-1.5 rounded-lg hover:bg-surface text-muted hover:text-foreground transition-all shrink-0">
      ...
      {sidebarOpen && (
        <div className="px-5 pt-5 pb-2">
          <p className="text-[9px] font-bold text-muted uppercase tracking-[0.12em]">Navigation</p>
        </div>
      )}
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.name} href={item.href} target={item.external ? "_blank" : undefined} rel={item.external ? "noreferrer" : undefined}
              ...
              {!sidebarOpen && (
                <div className="absolute left-full ml-3 px-3 py-1.5 bg-surface text-foreground text-xs font-semibold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap shadow-xl border border-line z-50">
                  {item.name}
                </div>
              )}
            </Link>
      ...
      <div className="p-3 border-t border-line space-y-2">
        ...
        <button onClick={onLogout}
          className={`flex items-center gap-3.5 rounded-xl text-danger/70 hover:text-danger hover:bg-danger/8 transition-all w-full
            ${sidebarOpen ? "px-3.5 py-2.5" : "px-0 py-2.5 justify-center"}`}
          title="Logout">
          <LogOut size={18} />
